 ...
 */
export const DEFAULT_COUNTY_RESOURCES = [
...
export const VENDOR_MASTER_CSV_URL = process.env.TEXAS_VENDOR_CSV_URL ?? 'https://comptroller.texas.gov/auto-data/purchasing/web_name.csv';
export const CASH_REPORT_ZIP_TMPL = process.env.TEXAS_CASH_REPORT_ZIP_TMPL ??
    'https://comptroller.texas.gov/transparency/reports/cash-report/{fy}/data/data.zip';
export function countyResourceList() {
    ...
    if (raw) {
        try {
            ...
            return parsed
                ...
                if (!Number.isFinite(fiscalYear) || !resourceId)
                    return null;
