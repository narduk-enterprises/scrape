...
import { CASH_REPORT_ZIP_TMPL, SOCRATA_EXPENDITURE_CATEGORIES, SOCRATA_OBJECT_CODES, VENDOR_MASTER_CSV_URL, cashReportFiscalYears, countyResourceList, paymentsFiscalYears, } from './sources.js';
const ALL = new Set([
    'payments',
    'county',
    'objects',
...
async function fetchText(url, timeoutMs = 180_000) {
...
function mapCountyRow(o, rowNumber) {
    ...
    const expType = pickField(o, 'major_spending_category', 'expenditure_category', 'expenditure_type');
...
function mapPaymentRow(o, rowNumber) {
...
function mapObjectRow(o, rowNumber) {
...
function mapCategoryRow(o, rowNumber) {
...
function mapVendorRow(o, rowNumber) {
    return {
        row_number: rowNumber,
...
async function tryDirectPaymentsCsv(fiscalYear) {
...
export async function runTexasPipeline(api, selection, snapshotDate) {
    const dry = process.env.TEXAS_DRY_RUN === '1';
    const want = (k) => selection.has('all') || selection.has(k);
    if (want('county')) {
        for (const { fiscalYear, resourceId } of countyResourceList()) {
            ...
            await retry(label, 3, async () => {
                ...
                if (text.trim().length < 50)
                    throw new Error('empty county CSV');
    ...
    if (want('cash')) {
        ...
        for (const fy of cashReportFiscalYears(yearsBack)) {
            ...
            await retry(label, 3, async () => {
                ...
                if (buf.length < 500)
                    throw new Error(`zip too small (${buf.length} bytes)`);
                ...
                for (const name of Object.keys(entries)) {
                    if (!/\.xlsx$/i.test(name) && !/\.xls$/i.test(name))
                        continue;
                ...
                if (allRows.length === 0)
                    throw new Error('no spreadsheet rows extracted');
        ...
        for (const fy of paymentsFiscalYears(Number(process.env.TEXAS_PAYMENTS_YEARS_BACK ?? '10'))) {
            ...
            await retry(label, 3, async () => {
                ...
                if (direct) {
                    ...
                    throw new Error('Set TEXAS_PAYMENTS_CSV_URL with {fy} or enable Playwright (unset TEXAS_PAYMENTS_PLAYWRIGHT=0)');
                }
                const checksum = sha256Hex(csvText);
                const { headers, rows } = parseCsvRows(csvText);
                if (rows.length === 0)
                    throw new Error('payments CSV has zero data rows');
                const objs = rowsToObjects(headers, rows);
...
export function parseTexasSelection(argv) {
    if (argv.length === 0)
        return new Set(['all']);
    const s = new Set();
    for (const a of argv) {
        ...
        if (ALL.has(k))
            ...
            log(`unknown dataset "${a}" — ignored (expected: ${[...ALL].join(', ')}, all)`);
    }
