 ...
 */
export async function downloadPaymentsCsvPlaywright(
  fiscalYear: number,
  timeoutMs: number,
): Promise<{ csvText: string; url: string; fileName: string }> {
  const { chromium } = await import('playwright')
  const dir = await mkdtemp(join(tmpdir(), 'tx-pay-'))
  const spendingUrl =
    process.env.TEXAS_SPENDING_PAGE_URL ?? 'https://comptroller.texas.gov/transparency/spending/'
  ...
  try {
    ...
    const trigger =
      process.env.TEXAS_SPENDING_DOWNLOAD_TRIGGER_SELECTOR ??
